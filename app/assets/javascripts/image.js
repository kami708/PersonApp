$(document).on('turbolinks:load', $(function() {
  ...
  function buildNewFile(img_num,num){
    var html = `<label class="image-uploader${num}" id="image-label-${img_num}" for="product_product_images_attributes_${img_num}_image" style="display: none;">
                  <div class="sell-upload-drop-box${num}">
                    <pre>ドロップアンドドロップ<br>またはクリックしてファイルをアップロード</pre>
                    <input type="file" class="upload-image" name="product[product_images_attributes][${img_num}][image]" id="product_product_images_attributes_${img_num}_image">
                  </div>
                </label>`
    return html;
  }
